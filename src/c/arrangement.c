...
int fill_valids(int *valid_rows, int last_row, int inter_val,
                unsigned char **inters, relabeling r, int numvalids,
                int *oldrows, int minvec) {
  int row_ptr = 0;
...
void search_aux(relabeling r, search_table table, unsigned char **inters,
  ...
  int *valid_rows =
      table.row_idx_slots[table.numrows + table.numcols +
                          1]; // malloc(sizeof(int) * table.num_valid_rows);
  int *valid_cols =
      table.col_idx_slots[table.numrows + table.numcols +
                          1]; // malloc(sizeof(int) * table.num_valid_cols);

  if (last_appended == ROW || last_appended == ROWCOL) {

    table.num_valid_rows =
        fill_valids(valid_rows, last_row, 0, inters, r, table.num_valid_rows,
                    table.valid_rows, minvec); // row_ptr;

    table.num_valid_cols =
        fill_valids(valid_cols, last_row, 1, inters, r, table.num_valid_cols,
                    table.valid_cols, minvec); // col_ptr;
  ...
  if (last_appended == COL || last_appended == ROWCOL) {
    ...
    table.num_valid_rows =
        fill_valids(valid_rows, last_col, 1, inters, r, table.num_valid_rows,
                    table.valid_rows, minvec); // row_ptr;

    table.num_valid_cols =
        fill_valids(valid_cols, last_col, 0, inters, r, table.num_valid_cols,
                    table.valid_cols, minvec); // col_ptr;
    table.valid_rows = valid_rows;
    table.valid_cols = valid_cols;
  }
  ...
  for (row_ptr = 0; row_ptr < table.num_valid_rows; row_ptr++) {
    ...
    if (max_elt != prev_max_elt) {
      ...
      while (col_ptr < table.num_valid_cols &&
             r.vecs[table.valid_cols[col_ptr]].elts[0] > max_elt) {
        col_ptr++;
      }
      if (col_ptr == table.num_valid_cols)
        break;
      col_start = col_ptr;
    ...
    int *new_valid_rows =
        table.row_idx_slots[new_table.numrows + new_table.numcols + 1];
    int *new_valid_cols =
        table.col_idx_slots[new_table.numrows + new_table.numcols + 1];

    new_table.num_valid_rows =
        fill_valids(new_valid_rows, roww, 0, inters, r,
                    new_table.num_valid_rows, new_table.valid_rows, minvec);
    new_table.num_valid_cols =
        fill_valids(new_valid_cols, roww, 1, inters, r,
                    new_table.num_valid_cols, new_table.valid_cols, minvec);
    new_table.valid_rows = new_valid_rows;
    new_table.valid_cols = new_valid_cols;
    if (!((new_table.numrows + new_table.num_valid_rows >= 6) &&
          (new_table.numcols + new_table.num_valid_cols >= 6))) {
      continue;
    }
  ...
  for (int i = row_ptr; i < table.num_valid_rows; i++) {
    ...
    if (max_elt < max_unmatched) {
      ...
      while (smallest_col < table.num_valid_cols &&
             table.valid_cols[smallest_col] <= roww) {
        smallest_col++;
      }
  ...
  for (int i = col_ptr; i < table.num_valid_cols; i++) {
    ...
    if (max_elt < max_unmatched) {
      ...
      while (smallest_row < table.num_valid_rows &&
             table.valid_rows[smallest_row] <= coll) {
        smallest_row++;
      }
...
int main(int argc, char *argv[]) {
  ...
  while (1) {
    ...
    opt = getopt_long(argc, argv, "f:s:", long_options, NULL);
    if (opt == -1)
      break;
